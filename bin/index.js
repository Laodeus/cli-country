...
const launch = country2digit => {
  ...
  country2digit.forEach(async elem => {
    ...
    response.data.forEach((respElem, id) => {
      console.log(
        `   element ${id} => ${respElem.localName} is ${respElem.date}`
      );
    });
  });
};
...
country2digit.length > 0
  ? launch(country2digit)
  : console.log("No valid country in argument...");
